...
@Constraint(validatedBy = ContentOrRatingRequiredValidator.class) // 실젝 검증 로직 연결
public @interface ContentOrRatingRequired {

    String message() default "내용 또는 평점 중 하나는 반드시 입력해야 합니다."; // 기본 에러 메시지

    Class<?>[] groups() default {}; // Bean validation 그룹
    Class<? extends Payload>[] payload() default {}; // 메타정보 전달용
}
