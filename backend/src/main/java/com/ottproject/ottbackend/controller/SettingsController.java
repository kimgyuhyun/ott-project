...
public class SettingsController { // 사용자 설정 관리
    ...
    private final UserProfileService userProfileService; // 사용자 프로필 조회 서비스
    ...
    @ApiResponse(responseCode = "200", description = "조회 성공")
    @GetMapping("/profile")
    public ResponseEntity<UserProfileDto> getProfile(HttpSession session) { // 세션 입력
        Long userId = securityUtil.requireCurrentUserId(session); // 사용자 확인(401 가능)
        return userProfileService.findProfile(userId).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound()
                .build());
    }
}
