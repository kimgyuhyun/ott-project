...
public class PlayerController { // 스트리밍/진행률
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "성공",
                    content = @Content(schema = @Schema(implementation = com.ottproject.ottbackend.dto.StreamUrlResponseDto.class))),
            @ApiResponse(responseCode = "401", description = "미인증"),
            @ApiResponse(responseCode = "403", description = "권한 없음")
    })
    @GetMapping("/api/episodes/{id}/stream-url") // 서명 URL 발급 엔드포인트
    public ResponseEntity<StreamUrlResponseDto> streamUrl(
