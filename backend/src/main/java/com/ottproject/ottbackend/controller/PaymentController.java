...
public class PaymentController { // 결제 컨트롤러 시작
    ...
    public ResponseEntity<PaymentCheckoutCreateSuccessResponseDto> checkout(
            @RequestBody PaymentCheckoutCreateRequestDto dto, HttpSession session) { // 요청 바디/세션 수신
    ...
    @Operation(
            summary = "결제 확정(클라이언트)",
            description = "결제창 성공 콜백에서 imp_uid로 결제를 아임포트 API에 재검증하고 즉시 확정/지급합니다. 웹훅 미도달을 대비한 주 경로입니다.") // Swagger 문서화
    ...
    public ResponseEntity<PaymentResultResponseDto> complete(
            @PathVariable Long paymentId, // 확정할 결제 ID
    ...
    public ResponseEntity<Void> setDefaultPaymentMethod(
            @PathVariable("id") Long id, HttpSession session) { // 기본 지정 엔드포인트
        Long userId = securityUtil.requireCurrentUserId(session); // 세션에서 사용자 ID 확인
    ...
    public ResponseEntity<Void> updatePaymentMethod(
            @PathVariable("id") Long id, // 수정 엔드포인트
    ...
    public ResponseEntity<List<PaymentHistoryItemDto>> history(
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) @RequestParam(value = "start", required = false)
                    LocalDateTime start, // 시작시각(선택)
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) @RequestParam(value = "end", required = false)
                    LocalDateTime end, // 종료시각(선택)
            HttpSession session // 세션 수신
            ) {
        Long userId = securityUtil.requireCurrentUserId(session); // 세션에서 사용자 ID 확인
    ...
    public ResponseEntity<PaymentResultResponseDto> checkPaymentStatus(
            @PathVariable Long paymentId, HttpSession session) { // 결제 상태 확인 API
        Long userId = securityUtil.requireCurrentUserId(session); // 세션에서 사용자 ID 확인
