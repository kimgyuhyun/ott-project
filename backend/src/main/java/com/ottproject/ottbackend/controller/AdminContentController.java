...
public class AdminContentController { // 컨트롤러 시작
    ...
    public ResponseEntity<List<AdminContentResponseDto>> listPublic(
            @Parameter(description = "컨텐츠 타입 (FAQ, BENEFIT, CTA)", required = true) @PathVariable
                    String type, // 유형 경로 변수
            @Parameter(description = "언어 코드 (기본값: ko)", required = false) @RequestParam(defaultValue = "ko")
                    String locale // 언어
            ) { // 메서드 시작
    ...
    public ResponseEntity<List<AdminContentResponseDto>> list(
            @Parameter(description = "컨텐츠 타입 (FAQ, BENEFIT, CTA)", required = true) @RequestParam String type, // 필수 유형
            @Parameter(description = "언어 코드", required = false) @RequestParam(required = false) String locale // 선택 언어
            ) { // 메서드 시작
    ...
    public ResponseEntity<AdminContentResponseDto> create(
            @Parameter(description = "컨텐츠 생성 정보", required = true) @RequestBody AdminContentRequestDto dto) { // 바디 입력
    ...
    public ResponseEntity<AdminContentResponseDto> update(
            @Parameter(description = "컨텐츠 ID", required = true) @PathVariable Long id,
            @Parameter(description = "컨텐츠 수정 정보", required = true) @RequestBody AdminContentRequestDto dto) { // 경로+바디
    ...
    public ResponseEntity<Void> delete(
            @Parameter(description = "컨텐츠 ID", required = true) @PathVariable Long id) { // 경로 변수
    ...
    public ResponseEntity<AdminContentResponseDto> publish(
            @Parameter(description = "컨텐츠 ID", required = true) @PathVariable Long id,
            @Parameter(description = "공개 여부 (true: 공개, false: 비공개)", required = true) @RequestParam
                    boolean value) { // query value
    ...
    public ResponseEntity<AdminContentResponseDto> position(
            @Parameter(description = "컨텐츠 ID", required = true) @PathVariable Long id,
            @Parameter(description = "새로운 순서 위치", required = true) @RequestParam int position) { // query position
