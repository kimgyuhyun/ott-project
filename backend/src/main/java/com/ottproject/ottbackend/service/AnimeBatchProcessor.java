...
public class AnimeBatchProcessor {
    ...
    private void processDirectors(Anime anime, Map<String, Object> jikanData) {
        ...
        Map<String, Director> existingDirectorMap =
                existingDirectors.stream().collect(Collectors.toMap(Director::getName, director -> director));
        ...
        log.info(
                "🎬 감독 처리 완료: {}명 (기존: {}, 신규: {})",
                managedDirectors.size(),
                existingDirectors.size(),
                newDirectorNames.size());
    }
    ...
    private void processGenres(Anime anime, Map<String, Object> jikanData) {
        ...
        Map<String, Genre> existingGenreMap =
                existingGenres.stream().collect(Collectors.toMap(Genre::getName, genre -> genre));
        ...
        log.info("🎭 장르 처리 완료: {}개 (기존: {}, 신규: {})", genres.size(), existingGenres.size(), newGenreNames.size());
    }
    ...
    private void processStudios(Anime anime, Map<String, Object> jikanData) {
        ...
        Map<String, Studio> existingStudioMap =
                existingStudios.stream().collect(Collectors.toMap(Studio::getName, studio -> studio));
        ...
        log.info("🏢 스튜디오 처리 완료: {}개 (기존: {}, 신규: {})", studios.size(), existingStudios.size(), newStudioNames.size());
    }
    ...
    private void processTags(Anime anime, Map<String, Object> jikanData) {
        ...
        Map<String, Tag> existingTagMap = existingTags.stream().collect(Collectors.toMap(Tag::getName, tag -> tag));
        ...
        anime.setTags(tags);
        log.info("🏷️ 태그 처리 완료: {}개 (기존: {}, 신규: {})", tags.size(), existingTags.size(), newTagNames.size());
    }
