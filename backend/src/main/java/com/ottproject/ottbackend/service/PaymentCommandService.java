...
public class PaymentCommandService { // 결제 쓰기 서비스
    ...
    public void processWebhook(HttpHeaders headers, String rawBody) {
        ...
        log.info("웹훅 이벤트 파싱 완료 - merchant_uid: {}, status: {}", event.providerSessionId, event.status);
        ...
        if (event.status == PaymentStatus.SUCCEEDED) {
            ...
            long expectedAmount = (paymentForVerify.getPrice() != null
                    ? paymentForVerify.getPrice().getAmount()
                    : 0L);
            boolean isValid = false;
            try {
                isValid =
                        paymentGateway.verifyPayment(event.providerPaymentId, event.providerSessionId, expectedAmount);
            } catch (Exception ex) {
                log.error(
                        "API 선재검증 중 예외 - imp_uid: {}, merchant_uid: {}",
            ...
            if (!isValid) {
                log.error(
                        "API 선재검증 실패 - imp_uid: {}, merchant_uid: {}",
                        event.providerPaymentId,
                        event.providerSessionId);
    ...
    private void verifyNonSuccessWebhook(PaymentWebhookEventDto event) {
        ...
        if (!r.found || toPaymentStatus(r.status) != event.status) {
            log.error(
                    "웹훅 재검증 실패 - merchant_uid: {}, 웹훅 주장: {}, 아임포트 실제: {}",
                    event.providerSessionId,
                    event.status,
                    r.status);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "웹훅 재검증 실패");
    ...
    private void processPaymentEvent(PaymentWebhookEventDto event) {
        ...
        log.info("결제 이벤트 처리 - paymentId: {}, 현재상태: {}, 웹훅상태: {}", payment.getId(), payment.getStatus(), event.status);
        ...
        try {
            ...
            if (body == null || body.isEmpty()) {
                throw new org.springframework.web.server.ResponseStatusException(
                        org.springframework.http.HttpStatus.BAD_REQUEST, "Empty webhook payload");
            }
            // Iamport 원문 키가 존재하면 내부 DTO로 수동 매핑
            if (body.containsKey("imp_uid") || body.containsKey("merchant_uid") || body.containsKey("status")) {
                com.ottproject.ottbackend.dto.PaymentWebhookEventDto dto =
                        new com.ottproject.ottbackend.dto.PaymentWebhookEventDto();
            ...
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.BAD_REQUEST, "Invalid webhook payload"); // 400
        }
    }
     ...
     * 체크아웃 생성
     */
    public PaymentCheckoutCreateSuccessResponseDto checkout(
            Long userId, PaymentCheckoutCreateRequestDto req) { // 체크아웃 생성
        ...
        String normalizedService =
                (req.paymentService == null ? null : req.paymentService.trim().toLowerCase());
    ...
    public void registerBillingKey(Long paymentId, Long userId, String customerUid) {
        ...
        PaymentMethod existing =
                paymentMethodRepository.findByUser_IdAndDeletedAtIsNullOrderByIsDefaultDescPriorityAsc(userId).stream()
    ...
    public void applyWebhookEvent(Long paymentId, PaymentWebhookEventDto event) { // 웹훅 반영
        ...
        if (event.providerSessionId != null
                ...
                && !event.providerSessionId.equals(payment.getProviderSessionId())) {
        ...
        if (event.status == PaymentStatus.SUCCEEDED) { // 성공
            ...
            subscriptionRepository
                    .findActiveEffectiveByUser(payment.getUser().getId(), MembershipSubscriptionStatus.ACTIVE, ts)
    ...
    public void refundIfEligible(Long userId, Long paymentId) { // 환불 엔드포인트 진입점
        ...
        try {
            ...
            // 3단계: 락 + 상태 재확인 + 확정
            self.confirmRefunded(
                    paymentId, target.amount(), rr.refundedAt != null ? rr.refundedAt : LocalDateTime.now());
            ...
            throw new ResponseStatusException(
                    HttpStatus.INTERNAL_SERVER_ERROR, "환불 처리 중 오류가 발생했습니다: " + e.getMessage());
        }
    }
    ...
    public RefundTarget claimRefund(Long userId, Long paymentId) {
        Payment payment = paymentRepository
                .findById(paymentId) // 결제 단건 조회(락 없는 빠른 경로)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "결제가 존재하지 않습니다.")); // 400
        ...
        int totalWatched = playerProgressReadService.sumWatchedSecondsSincePaidEpisodes(
                userId, payment.getPaidAt()); // 4화 이상 누적 시청 초 합
        ...
        return new RefundTarget(
                payment.getProviderPaymentId(), payment.getPrice().getAmount());
    }
    ...
    public void confirmRefunded(Long paymentId, long refundedAmount, LocalDateTime refundedAt) {
        ...
        idempotencyKeyRepository
                .findByKeyValue(refundIdempotencyKey(paymentId))
        ...
        subscriptionRepository
                .findActiveEffectiveByUser(ownerId, MembershipSubscriptionStatus.ACTIVE, now)
                .ifPresent(sub -> {
    ...
    public RefundTarget loadRefundTarget(Long paymentId) {
        return paymentRepository
                .findById(paymentId)
                .map(p ->
                        new RefundTarget(p.getProviderPaymentId(), p.getPrice().getAmount()))
                .orElse(null);
    }
     ...
     */
    private void markSucceededAndProvision(
            Payment payment, String providerPaymentId, String receiptUrl, LocalDateTime paidAt) {
        ...
        try {
            ...
            log.info(
                    "멤버십 구독 생성 완료 - userId: {}, planCode: {}", payment.getUser().getId(), subscribeDto.planCode);
        } catch (ResponseStatusException e) {
    ...
    public ReconcileTarget prepareReconcile(Long paymentId) {
        ...
        if (payment.getProviderSessionId() != null
                && payment.getProviderSessionId().startsWith("proration_")) {
            return null;
        }
        return new ReconcileTarget(
                payment.getProviderSessionId(), payment.getUser().getId());
    }
