...
public class TmdbApiService {
    ...
    public TmdbAnimeData searchAnime(String title) {
        ...
        try {
            // TMDB API 검색 요청
            String searchUrl = baseUrl + "/search/movie" + "?api_key="
                    + apiKey + "&query="
                    + title.trim() + "&language=ko-KR"
                    + "&include_adult=false";
    ...
    private TmdbAnimeData parseTmdbAnimeData(Map<String, Object> data) {
        try {
            ...
            log.info("TMDB 데이터 파싱 완료: {} (한국어: {})", animeData.getTitle(), hasKoreanData);
    ...
    public static class TmdbAnimeData {
        ...
        public boolean isHasKoreanData() {
            return hasKoreanData;
        }

        public void setHasKoreanData(boolean hasKoreanData) {
            this.hasKoreanData = hasKoreanData;
        }
    }
}
