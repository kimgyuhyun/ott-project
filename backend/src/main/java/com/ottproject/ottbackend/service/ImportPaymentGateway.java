...
public class ImportPaymentGateway implements PaymentGateway { // IMPORT 구현 시작
    ...
    public CheckoutSession createCheckoutSession(
            ...
            long amount) { // 세션 생성(prepare-only)
        ...
        callWithToken(
                token -> { // 금액을 고정하는 상태 변경이라 401 재시도 대상이 아니다
                    HttpHeaders h = bearer(token); // 인증 헤더
                    h.setContentType(MediaType.APPLICATION_JSON); // JSON 바디
                    return rest.exchange(
                            apiBase + "/payments/prepare",
                            HttpMethod.POST,
                            new HttpEntity<>(prepareBody, h),
                            String.class); // 결제 준비 등록
                });
    ...
    public RefundResult issueRefund(String providerPaymentId, long amount) { // 환불 실행
        ...
        callWithToken(
                token -> { // 환불 실행이라 401 재시도 대상이 아니다 - 다시 부르면 두 번 나갈 수 있다
                    HttpHeaders h = bearer(token); // 인증 헤더
                    h.setContentType(MediaType.APPLICATION_JSON); // JSON 바디
                    return rest.exchange(
                            apiBase + "/payments/cancel",
                            HttpMethod.POST,
                            new HttpEntity<>(body, h),
                            java.util.Map.class); // 호출
                });
    ...
    public RefundStatus findRefundStatus(String providerPaymentId) {
        ...
        try {
            ...
            java.util.Map<String, Object> res =
                    (java.util.Map<String, Object>) response.getBody().get("response");
    ...
    @Override
    public ChargeResult chargeWithSavedMethod(
            String providerCustomerId,
            String providerMethodId,
            String merchantUid,
            long amount,
            String currency,
            String description) { // 저장수단 청구
        ...
        String body = String.format(
                "{\"customer_uid\":\"%s\",\"merchant_uid\":\"%s\",\"amount\":%d,\"name\":\"%s\"}",
                providerMethodId, merchantUid, amount, description == null ? "Subscription" : description); // 바디
        ResponseEntity<String> res;
        try {
            res = rest.exchange(
                    apiBase + "/subscribe/payments/again",
                    HttpMethod.POST,
                    new HttpEntity<>(body, h),
                    String.class); // 호출
            ...
            throw new ChargeException(
                    FailureType.SOFT_DECLINE, "HTTP_" + e.getStatusCode().value(), e.getMessage());
            ...
            throw new ChargeException(
                    ...
                    "NO_RESPONSE",
                    "재청구 응답 확인 불가 - merchant_uid: " + merchantUid + ", " + e.getMessage());
        }
    ...
    public boolean verifyPayment(String impUid, String merchantUid, long expectedAmount) {
        try {
            ...
            ResponseEntity<java.util.Map> response = callWithTokenRetryingOnce(
                    token -> rest.exchange(url, HttpMethod.GET, new HttpEntity<>(bearer(token)), java.util.Map.class));
    ...
    private String fetchAndCacheAccessToken() {
        ...
        ResponseEntity<TokenResponse> res = rest.exchange(
                apiBase + "/users/getToken",
                HttpMethod.POST,
                new HttpEntity<>(body, headers),
                TokenResponse.class); // 호출
        ...
        if (token.now > 0 && lifetimeSeconds > TOKEN_REFRESH_MARGIN_SECONDS) {
            long usableUntilMillis =
                    System.currentTimeMillis() + (lifetimeSeconds - TOKEN_REFRESH_MARGIN_SECONDS) * 1000L;
