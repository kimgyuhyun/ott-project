...
public class SimpleAnimeDataCollectorService {
    ...
    @Transactional(
            ...
            timeout = 300,
            isolation = Isolation.READ_COMMITTED) // 5분 타임아웃 (개발 환경)
    public boolean collectAnime(Long malId) {
        ...
        try {
            ...
            try {
                ...
                if (anime == null) {
                    log.error(
                            "❌ Anime 엔티티 생성 실패: MAL ID {} (소요시간: {}ms) - null 반환",
                            malId,
                            System.currentTimeMillis() - startTime);
                    return false; // finally 블록에서 ThreadLocal 정리됨
                }
                ...
                log.error(
                        "❌ Anime 엔티티 생성 실패: MAL ID {} (소요시간: {}ms) - {}",
                        malId,
                        System.currentTimeMillis() - startTime,
                        e.getMessage(),
                        e);
            ...
            if (isDuplicate) {
                log.info(
                        "⚠️ 이미 존재하는 애니메이션: {} (MAL ID: {}, 소요시간: {}ms)",
                        duplicateTitle,
                        malId,
                        System.currentTimeMillis() - startTime);
                return false; // finally 블록에서 ThreadLocal 정리됨
            ...
            try {
                ...
                log.info(
                        "💾 애니메이션 DB 저장 완료: ID {} (MAL ID: {}, 소요시간: {}ms)",
                        anime.getId(),
                        malId,
                        System.currentTimeMillis() - startTime);
            } catch (org.springframework.dao.DataIntegrityViolationException e) {
                log.warn(
                        "⚠️ 중복 제약조건 위반: {} (MAL ID: {}, 소요시간: {}ms) - 다른 스레드에서 이미 저장됨",
                        anime.getTitle(),
                        malId,
                        System.currentTimeMillis() - startTime);
                return false; // finally 블록에서 ThreadLocal 정리됨
    ...
    private Set<VoiceActor> processVoiceActorsBatch(Set<VoiceActor> voiceActors) {
        ...
        Map<String, List<VoiceActor>> existingVoiceActorMap =
                existingVoiceActors.stream().collect(Collectors.groupingBy(VoiceActor::getName));
        ...
        for (VoiceActor voiceActor : voiceActors) {
            ...
            if (name != null && !name.trim().isEmpty()) {
                ...
                if (malId != null) {
                    // MAL ID로 먼저 시도
                    var byMal = existingVoiceActors.stream()
                            .filter(v -> malId.equals(v.getMalId()))
                            .findFirst();
                    if (byMal.isPresent()) {
        ...
        if (!newVoiceActors.isEmpty()) {
            try {
                Set<VoiceActor> savedVoiceActors =
                        new java.util.HashSet<>(voiceActorRepository.saveAll(newVoiceActors));
    ...
    private Set<Character> processCharactersBatch(Set<Character> characters) {
        ...
        Map<String, List<Character>> existingCharacterMap =
                existingCharacters.stream().collect(Collectors.groupingBy(Character::getName));
        ...
        for (Character character : characters) {
            ...
            if (name != null && !name.trim().isEmpty()) {
                ...
                boolean matchedByMal = false;
                if (malId != null) {
                    var byMal = existingCharacters.stream()
                            .filter(c -> malId.equals(c.getMalId()))
                            .findFirst();
                    if (byMal.isPresent()) {
    ...
    public static class CollectionResult {
        ...
        public int getAdultContentCount() {
            return adultContentCount;
        }
        ...
        public int getTotalProcessed() {
            return successCount + adultContentCount + errorCount;
        }

        @Override
        public String toString() {
            return String.format(
                    "성공: %d, 19금 제외: %d, 오류: %d, 총 처리: %d",
        ...
        if (charactersDto.getData() != null) {
            for (var item : charactersDto.getData()) {
                ...
                if (item.getCharacter() != null) {
                    ...
                    if (item.getCharacter().getImages() != null
                            && item.getCharacter().getImages().getJpg() != null) {
                        Map<String, Object> jpg = new java.util.HashMap<>();
                        jpg.put(
                                "image_url",
                                item.getCharacter().getImages().getJpg().getImage_url());
                        img.put("jpg", jpg);
                    }
     ...
     */
    public Set<Character> mapToExistingCharacters(
            Map<String, Object> charactersData, CharacterRepository characterRepository) {
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> charactersList =
                (List<Map<String, Object>>) charactersData.getOrDefault("characters", java.util.List.of());
     ...
     */
    public Set<VoiceActor> mapToExistingVoiceActors(
            Map<String, Object> charactersData, VoiceActorRepository voiceActorRepository) {
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> charactersList =
                (List<Map<String, Object>>) charactersData.getOrDefault("characters", java.util.List.of());
        ...
        for (Map<String, Object> item : charactersList) {
            @SuppressWarnings("unchecked")
            List<Map<String, Object>> vaList =
                    (List<Map<String, Object>>) item.getOrDefault("voice_actors", java.util.List.of());
     ...
     */
    @Transactional
    public void upsertCharacterVoiceActorJoins(
        ...
        if (existingCharacters.isEmpty() || existingVoiceActors.isEmpty()) {
            log.info(
                    "캐릭터-성우 조인 스킵: character={}, voiceActor={}", existingCharacters.size(), existingVoiceActors.size());
            return;
        }
        ...
        for (Map<String, Object> item : charactersList) {
            ...
            java.util.Set<VoiceActor> current = character.getVoiceActors() != null
                    ? new java.util.HashSet<>(character.getVoiceActors())
            ...
            if (current.size() > before) {
                ...
                savedCount++;
                log.debug(
                        "캐릭터-성우 조인 저장: characterId={}, before={}, after={}", character.getId(), before, current.size());
            } else {
    ...
    public void upsertAnimeVoiceActorJoins(Long animeId, Set<Long> voiceActorIds) {
        ...
        java.util.Set<VoiceActor> current = anime.getVoiceActors() != null
                ? new java.util.HashSet<>(anime.getVoiceActors())
                : new java.util.HashSet<>();
        int before = current.size();
    ...
    public void upsertAnimeCharacterJoins(Long animeId, Set<Long> characterIds) {
        ...
        java.util.Set<Character> current = anime.getCharacters() != null
                ? new java.util.HashSet<>(anime.getCharacters())
                : new java.util.HashSet<>();
        int before = current.size();
    ...
    public void processDirectorsInBackground(Long animeId, Long malId) {
        ...
        try {
            ...
            log.info(
                    "🔍 Staff API 응답: MAL ID {}, staffItems 크기: {}",
                    malId,
                    staffItems != null ? staffItems.size() : "null");
            ...
            java.util.Map<String, Director> existingMap =
                    existing.stream().collect(java.util.stream.Collectors.toMap(Director::getName, d -> d));
