...
public class AniList {
    ...
    private AnimeStatus status; // 방영 상태 (방영중, 완결, 방영예정, 방영중단)
    ...
    /**
    장르와의 다대다 관계
    하나의 애니는 여러 장르를 가질 수 있고, 하나의 장르는 여러 애니에 속할 수 있음
    중간 테이블(ani_list_genres)를 통해 관계 관리
     */

    @ManyToMany(fetch = FetchType.LAZY, cascade = {CascadeType.PERSIST, CascadeType.MERGE}) // 지연 로딩으로 성능최적화, 영속성 전이 설정
    @JoinTable(
    ...
    @Builder.Default // 빌더 패턴에서 기본값 설정
    private Set<Genre> genres = new HashSet<>(); // 장르 목록 (Set 으로 중복 방지)

    /**
     * 제작사와의 다대다 관계
     * 하나의 애니는 여러 제자가가 참여할 수 있고, 하나의 제작사는 여러 애니를 제작
     * 중간 테이블(ani_list_studios) 를 통해 관계 관리
     */
    ...
    @Builder.Default // 빌더 패턴에서 기본값 설정정
    private Set<Studio> studios = new HashSet<>(); // 제작사 목록 (Set 으로 중복 방지)
