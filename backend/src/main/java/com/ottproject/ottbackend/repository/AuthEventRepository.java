...
public interface AuthEventRepository extends JpaRepository<AuthEvent, Long> {
    ...
    long countByTypeBetween(
            @Param("type") AuthEventType type, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    /**
     * 특정 유형의 기간 내 고유 사용자 수 (DAU: 일일 활성 사용자 산출용)
     * - userId 가 null 인(미식별) 이벤트는 제외한다.
     */
    @Query("SELECT COUNT(DISTINCT e.userId) FROM AuthEvent e " + "WHERE e.eventType = :type AND e.userId IS NOT NULL "
            + "AND e.occurredAt >= :start AND e.occurredAt < :end")
    long countDistinctUsersByTypeBetween(
            @Param("type") AuthEventType type, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
