...
| 파일 | 무엇 |
|---|---|
| `RESULTS.md` | **실제로 돌린 결과.** 이 문서는 방법, 저기는 실측치 |
...
```bash
k6 run -e TEST=stress -e LT_PASSWORD=... -e BACKENDS=http://127.0.0.1:8090,http://127.0.0.1:8093 loadtest/main.js
```

사람 수가 아니라 **초당 요청수**를 직접 올린다(100 → 500 → 1000 → 2000 → 4000 이터레이션/초).
`knee` 는 VU 1명이 5초에 1회만 쏘는 closed-loop 이라 1500 VU 로도 417 RPS 가 상한이라 한계를 못 잰다.
진행률 저장 p95 가 500ms 를 넘거나 에러율이 1% 를 넘으면 자동 중단된다.

로그인은 `setup()` 에서 미리 끝내고 VU 는 세션을 빌려 쓴다(`LT_SESSIONS`, 기본 200).
부하 구간에서 로그인하면 "느려짐 → VU 증설 → BCrypt 폭주 → 더 느려짐" 되먹임으로 측정이 무효가 된다.
실제로 이걸로 두 번 날렸다 — RESULTS.md 의 "측정 방법에서 걸린 함정" 참고.
