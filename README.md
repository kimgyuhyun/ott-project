...
![아키텍처 요약](docs/images/architecture.svg)

그림 번호 → [2 아웃박스](#2-결제-부수효과-outbox--kafka) · [4 서명 URL](#4-재생-권한--서명-url) · [7 write-back](#7-시청-진행률-write-back--포화점-15배) · [8 아웃바운드 통제](#8-아웃바운드-통제-목적지-허용목록-프록시) · 네트워크 분리 상세는 [아키텍처](#아키텍처)
...
- 둘 다 없으면 **거부**(403)합니다. 브라우저는 상태 변경 요청에 `Origin`을 붙이므로 정상 사용자는 막히지 않고, 헤더를 뺀 요청은 출처를 확인할 수 없어 통과시킬 근거가 없습니다. 출처 헤더를 보내지 않는 결제 웹훅 경로만 검사에서 뺍니다.
