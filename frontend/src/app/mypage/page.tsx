...
function MyPageContent() {
  ...
  const [wantList, setWantList] = useState<FavoriteAnime[]>([]);
  const [bingeList, setBingeList] = useState<BingeWatch[]>([]);
  ...
  const [userStats, setMypageStats] = useState<MypageStats | null>(null);
  ...
  const [commentSort, setCommentSort] = useState<"latest" | "likes" | "oldest">(
    "latest",
  );

  // 멤버십 상태
  const { userMembership, isLoading: isLoadingMembership } =
    useMembershipData();
    ...
    const loadUserData = async () => {
      try {
        ...
        const [
          ...
          getUserRecentAnime().catch((e) => {
            if ((e as Error & { status?: number })?.status === 401)
              return { items: [] };
            throw e;
          }),
          getUserWantList().catch((e) => {
            if ((e as Error & { status?: number })?.status === 401)
              return { items: [] };
            throw e;
          }),
          getUserBingeList().catch((e) => {
            if ((e as Error & { status?: number })?.status === 401) return [];
            throw e;
          }),
        ...
        // 시청 기록에 애니메이션 제목 추가 (RecentAnimeWatch → 상세 조인으로 title/poster 보강)
        const watchHistoryList = historyData.items;
        ...
        const enrichedWatchHistory: WatchHistoryItem[] = await Promise.all(
          watchHistoryList.map(async (item): Promise<WatchHistoryItem> => {
            ...
            try {
              ...
              return {
                ...item,
                aniId: animeId,
                title: animeDetail.title || "제목 없음",
                posterUrl: animeDetail.posterUrl || undefined,
              };
            } catch (e) {
              console.log(
                `애니메이션 상세 조회 실패 (ID: ${animeId}):`,
        ...
        // 보고싶다 목록 - FavoriteAnimeDto 가 이미 aniId/title/posterUrl 을 제공
        const wantListItems = wantListData.items;
        setWantList(wantListItems);
        setBingeList(bingeListData);
        setMypageStats(statsData);

        // 탭별 카운트 업데이트
        tabs[0].count = enrichedWatchHistory.length;
        tabs[1].count = wantListItems.length;
        tabs[2].count = 0; // 구매한 작품은 별도 API 필요
        tabs[3].count = bingeListData.length;
      } catch (err) {
    ...
    const loadActivity = async () => {
      try {
        ...
        if (activityTab === "ratings") {
          if (myRatings == null) {
            const data = await getMyRatings().catch(
          ...
          if (myReviews == null) {
            const data = await getMyReviews().catch(
              (e: Error & { status?: number }) => {
                if (e?.status === 401) return [];
                throw e;
              },
          ...
          if (myComments == null) {
            const data = await getMyComments().catch(
              (e: Error & { status?: number }) => {
                if (e?.status === 401) return [];
                throw e;
              },
    ...
    try {
      if (activeTab === "recent") {
        ...
        setWatchHistory((prev) =>
          prev.filter((anime) => !selectedAnimeIds.has(anime.aniId || 0)),
        );
      } else if (activeTab === "want") {
        ...
        if (activityTab === "ratings") {
          ...
          setMyRatings(
            (prev) =>
              prev?.filter(
                (rating) => !selectedAnimeIds.has(rating.animeId || 0),
              ) || [],
          );
        } else if (activityTab === "reviews") {
          ...
          setMyReviews(
            (prev) =>
              prev?.filter(
                (review) => !selectedAnimeIds.has(review.animeId || 0),
              ) || [],
          );
        } else if (activityTab === "comments") {
          ...
          setMyComments(
            (prev) =>
              prev?.filter(
                (comment) => !selectedAnimeIds.has(comment.commentId || 0),
              ) || [],
          );
        }
      }
  ...
  const handleAnimeClick = async (anime: {
    ...
    const position =
      typeof anime?.positionSec === "number" && anime.positionSec > 0
        ? anime.positionSec
        : 0;
                  ...
                  {userMembership ? (
                    <>
                      <h3 className={styles.bannerTitle}>
                        {getKoreanPlanName(
                          userMembership.planName || userMembership.planCode,
                        )}{" "}
                        멤버십 이용중입니다.
                      </h3>
                    </>
            ...
            <div className={styles.mainContentArea}>
              {viewMode === "archive" ? (
                        ...
                        {!isDeleteMode ? (
                          ...
                          <div className={styles.selectAllContainer}>
                            <input
                              type="checkbox"
                              checked={
                                selectedAnimeIds.size === watchHistory.length &&
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div
                                      ...
                                      style={{
                                        backgroundImage: anime.posterUrl
                                          ? `url(${anime.posterUrl})`
                                          : "none",
                                        backgroundSize: "cover",
                                        backgroundPosition: "center",
                                        backgroundColor: anime.posterUrl
                                          ? "transparent"
                                          : "#323232",
                                      }}
                                    ></div>
                                    ...
                                    {anime.episodeNumber && (
                                      <p className={styles.animeEpisode}>
                                        {anime.episodeNumber}화
                                      </p>
                                    )}
                                  </div>
                                );
                        ...
                        {!isDeleteMode ? (
                          ...
                          <div className={styles.selectAllContainer}>
                            <input
                              type="checkbox"
                              checked={
                                selectedAnimeIds.size === wantList.length &&
                                wantList.length > 0
                              }
                              onChange={handleSelectAll}
                              className={styles.selectAllCheckbox}
                        ...
                        {!isDeleteMode ? (
                          ...
                          <div className={styles.selectAllContainer}>
                            <input
                              type="checkbox"
                              checked={
                                selectedAnimeIds.size === bingeList.length &&
                                bingeList.length > 0
                              }
                              onChange={handleSelectAll}
                              className={styles.selectAllCheckbox}
                ...
                <div className={styles.tabContainer}>
                  <div className={styles.tabMenu}>
                    <div className={styles.tabButtons}>
                      {(["ratings", "reviews", "comments"] as const).map(
                        (tab: "ratings" | "reviews" | "comments") => (
                          <button
                            key={tab}
                            onClick={() => {
                              setActivityTab(tab);
                    ...
                    {activityTab === "ratings" && (
                      <div>
                        {!isDeleteMode ? (
                          ...
                          <div className={styles.selectAllContainer}>
                            <input
                              type="checkbox"
                              checked={
                                selectedAnimeIds.size ===
                                  (myRatings?.length || 0) &&
                                (myRatings?.length || 0) > 0
                              }
                              onChange={handleSelectAll}
                              className={styles.selectAllCheckbox}
                            />
                        ...
                        {Array.isArray(myRatings) && myRatings.length > 0 ? (
                          <div className={styles.animeGrid}>
                            {myRatings.map((item: MyRating, idx: number) => {
                              const aniId = item.animeId || 0;
                              const isSelected = selectedAnimeIds.has(aniId);
                      ...
                      <div>
                        ...
                        {Array.isArray(myReviews) && myReviews.length > 0 ? (
                          <div className={styles.reviewList}>
                            {myReviews.map((item: MyReview, idx: number) => {
                              const aniId = item.animeId || 0;
                              const isSelected = selectedAnimeIds.has(aniId);
                                ...
                                >
                                  ...
                                  <div className={styles.reviewBody}>
                                      ...
                                      <div className={styles.reviewFooter}>
                                        ...
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            const willLike = !likedReviews.has(
                                    ...
                                    <div className={styles.reviewPosterWrap}>
                                      {isDeleteMode && isSelected && (
                                        <div
                                          className={styles.thumbCheckOverlay}
                                        >
                                          ✓
                                        </div>
                                      )}
                      ...
                      <div>
                        <div className={styles.commentListHeader}>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setCommentSort(
                                e.target.value as "latest" | "likes" | "oldest",
                              )
                            }
                          >
                        ...
                        {Array.isArray(myComments) && myComments.length > 0 ? (
                          (() => {
                            const sorted = [...myComments].sort(
                              (a: MyComment, b: MyComment) => {
                            ...
                            return (
                              <div
                                ...
                                aria-label="내 댓글 목록"
                              >
                                {sorted.map((item: MyComment, idx: number) => {
                                  const commentId = item.commentId || 0;
                                  const isSelected =
                                    selectedAnimeIds.has(commentId);
                                    ...
                                    >
                                      ...
                                      <div className={styles.commentTopRow}>
                                        <div className={styles.commentTitleBox}>
                                          ...
                                          >
                                            <span
                                              className={styles.commentAniTitle}
                                            >
                                              {item.title}
                                            </span>
                                          </div>
                                        ...
                                        >
                                          ...
                                          >
                                            <path
                                              ...
                                              fill={
                                                likedComments.has(
                                                  `${item.targetType}-${item.commentId}`,
                                                )
                                                  ? "#8b5cf6"
                                                  : "currentColor"
                                              }
                                      ...
                                      <div
                                        className={styles.commentEpisodeThumb}
                                      >
                                        {isDeleteMode && isSelected && (
                                          <div
                                            className={styles.thumbCheckOverlay}
