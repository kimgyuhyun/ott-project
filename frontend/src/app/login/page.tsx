...
export default function LoginPage() {
  ...
  useEffect(() => {
    // React 훅: 컴포넌트가 랜더링 된 후 실행하는 함수, 페이지 로드 시 한 번 실행(의존성 배열[])
    ...
    const loadLoginUrls = async () => {
      ...
      try {
        // API 호출 시도
        const res = await fetch("/api/oauth2/login-urls", {
          credentials: "include",
        });
        ...
        if (data && data.loginUrls) {
          // 만약 data가 있고 그 객체 안의 loginUrls 속성이 있으면 실행
          // 만약 false가 뜨면 catch로 이동은 안하고 아래 기본값 설정 코드만 실행됨
          setOauthUrls({
            // 상태 변경 함수 호출 // 콜백 함수 안에서 상태를 재정의하는 방식
            ...
            google:
              data.loginUrls.google || `/login/oauth2/authorization/google`,
        ...
        // res.ok = false -> !로 true로 치환되서 조건성립 예외처리로 catch로 이동
        // 실패 시 기본값으로 폴백
        // FallBack: 대체/백업이란 의미 코드에서 APi 실패 시 기본값으로 대체
      }
      setOauthUrls({
        // data가 null이거나 loginUrls 요소가 없으면 여기로 바로옴
  ...
  return (
    <main className={styles.loginContainer}>
      ...
      <PosterWall />{" "}
      {/* import PosterWall from "@/components/auth/PosterWall"; 컴포넌트 사용*/}
      <Modal
        open={open}
      ...
      >
        ...
        <div className={styles.loginModal}>
          {" "}
          {/*loginModal css 클래스 적용 */}
          <div className={styles.loginLogo}>LAPUTA</div>{" "}
          {/*loginLogo css 클래스 적용 */}
          <div className={styles.loginDescription}>
            {" "}
            {/*loginDescription css 클래스 적용 */}
            동시방영 신작부터 역대 인기작까지
            <br />한 곳에서 편-안하게!
          </div>{" "}
          {/*loginDescription css 클래스 적용 */}
          ...
          />
          {/* 소셜 버튼 152*/}
          {/* 소셜 로그인 종류가 email이라는 뜻 153*/}
          {/* 버튼에 표시될 텍스트 154 */}
          {/* 버튼 클릭 시 handleEmailClick 함수 호출하고 이메일폼이 오픈됨 155*/}
          <div className={styles.loginDivider}>또는</div>{" "}
          {/*loginDivider css 클래스 적용 */}
          <div className={styles.socialButtonsContainer}>
            {" "}
            {/*socialButtonsContainer css 클래스 적용 소셜 버튼들을 감싸는 컨테이너임*/}
            ...
            */}
            ...
            <a
              ...
              aria-label="naver"
              className={`${styles.socialButton} ${styles.socialButtonNaver}`}
            >
              <Image
                alt="naver"
                src="/icons/naver.svg"
      ...
      {showEmailForm && ( // 조건부 렌더링할때 사용 shwEmailForm이 true일떄 렌더링
        ...
        그리고 onClose에 컴포넌트 닫는 함수를 넘겨야 esc키로 닫을수가 있음
        */}
          <EmailAuthForm
