      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            ...
            (userData.attributes || userData.principal)
          ) {
            // OAuth2 사용자 정보를 AuthContext 형식으로 변환
            const user = {
              id:
                userData.id ||
                userData.attributes?.userId ||
                userData.attributes?.id ||
                userData.principal?.attributes?.userId ||
                "unknown",
              ...
              username:
                ...
                userData.principal?.attributes?.userName ||
                userData.principal?.attributes?.name,
              email:
                userData.email ||
                userData.attributes?.userEmail ||
                userData.attributes?.email ||
                userData.username,
              profileImage:
                userData.attributes?.picture ||
                userData.principal?.attributes?.picture ||
                undefined,
            };
            ...
            const attrIsNew =
              userData.attributes?.isNewUser ?? userData.isNewUser;
            const isNewByAttr = attrIsNew === true || attrIsNew === "true";
            const needsNickname =
              !user.username || String(user.username).trim().length < 2;
