...
export default function PlanChangeModal({
  ...
  const [changeResult, setChangeResult] =
    useState<MembershipPlanChangeResponse | null>(null);
  ...
  return (
    <>
        ...
        <div className={styles.modalContent}>
          ...
          <h2 className={styles.modalTitle}>
            {changeType === "UPGRADE" ? (
              <>
                지금부터{" "}
                <span className={styles.planNameHighlight}>
                  {translatePlanName(targetPlan.name)}
                </span>
                을 사용하시겠어요?
              </>
            ) : (
          ...
          <div className={styles.infoList}>
              ...
              <>
                <div className={styles.infoItem}>
                  ...
                  <span className={styles.infoText}>
                    이번 달은{" "}
                    <span className={styles.planNameHighlight}>
                      {translatePlanName(currentPlan.name)}
                    </span>{" "}
                    혜택이 유지됩니다.
                  </span>
                </div>
            ...
            <div className={styles.infoItem}>
              ...
              <span className={styles.infoText}>
                {changeType === "DOWNGRADE" ? (
                  <>
                    {user?.username || "사용자"} 프로필을 제외한 나머지 프로필은
                    비활성화 처리되며, 관련된 정보(등급, 보고싶다, 재생기록
                    등)을 이용할 수 없게 됩니다.
                  </>
                ) : (
                  <>
                    {user?.username || "사용자"} 프로필을 제외한 나머지 프로필이
                    활성화 처리되며, 관련된 정보(등급, 보고싶다, 재생기록 등)을
                    이용할 수 있게 됩니다.
                  </>
                )}
              </span>
            ...
            <div className={styles.infoItem}>
              <div className={styles.checkIcon}>✓</div>
              <span className={styles.infoText}>
                다운로드, 소장·대여를 포함한 구매 내역, 별점, 게시글은 삭제되지
                않습니다.
              </span>
            </div>
          </div>
      ...
      <PlanChangeSuccessModal
        ...
        newPlan={targetPlan}
        effectiveDate={
          changeResult?.effectiveDate || userMembership?.nextBillingAt || ""
        }
      />
    </>
  );
