...
import { getBulkEpisodeProgress } from "@/lib/api/player"; // 에피소드별 시청 진행률(벌크)
import { AnimeDetail, AnimeListItem, Episode } from "@/types/anime"; // 애니 도메인 타입(캐노니컬)
...
export default function AnimeDetailModal({ anime, isOpen, onClose }: AnimeDetailModalProps) {
  ...
  const [detail, setDetail] = useState<AnimeDetail>(anime);
  ...
  const [similarAnimes, setSimilarAnimes] = useState<AnimeListItem[]>([]);
  ...
  const handleRatingChange = (newRating: number) => {
    ...
    setDetail((prev) => ({ ...prev, rating: newRating }));
  ...
  const handleDeleteWatchHistory = async () => {
    ...
    try { // try-catch 문으로 예외 처리를 함
      ...
      await deleteFromBinge(Number(detail?.aniId));
      // detail는 detail을 any 타입으로 타입 단언한것이고
      ...
      // detail?.aniId ?? detail?.id 이 부분을 해석하면
  ...
  const loadSimilarAnimes = async () => {
    ...
    try { // try 블록은 예외가 발생할 수 있는 코드 블록임
      ...
      const filtered = rawItems.filter((a) => Number(a?.aniId) !== baseId);
  ...
  // true인 경우는 detail.epsodes as Episode[]
