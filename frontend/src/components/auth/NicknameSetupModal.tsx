...
export default function NicknameSetupModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        getErrorMessage(err) ?? "닉네임 설정 중 오류가 발생했습니다.";
      setError(message);
    } finally {
      setIsLoading(false);
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="환영합니다!">
      <div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* 버튼들 */}
          <div>
            <button type="submit" disabled={isLoading || !nickname.trim()}>
              {isLoading ? (
                <span>
                  <div></div>
                  설정 중...
        ...
        {/* 추가 안내 */}
        <div>
          <p>닉네임은 언제든지 설정 페이지에서 변경할 수 있습니다.</p>
        </div>
      </div>
    </Modal>
  );
