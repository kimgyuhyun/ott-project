...
import {
  ...
  verifyCode,
} from "@/lib/api/auth"; // lib 에서 가져온 API 함수들들
...
interface EmailAuthFormProps {
  // EmailAuthForm 컴포넌트가 받을 props의 타입을 정의 , props 객체 정의
...
export default function EmailAuthForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ===는 값과 타입 모두 비교하는것 // try는 handleSubmit 실행 시 바로 try 블록을 실행함
      ...
      if (mode === "login") {
        ...
        if (user) {
          // 만약 user 객체가 있으면, (null이 아니고, undefined도 아니면) // 여기는 user 객체가 있을때만 실행됨
          ...
          setAuthUser({
            // useAuth()로 가져온 login 함수를 구조분해 할당 받은 변수로 사용
        ...
        // mode가 login이 아니라 register 일 때 실행
        if (registerStep === "email") {
          // 만약 회원가입 단계가 email 단계일 때 실행
          ...
          if (isDuplicate) {
            // 만약 isDublicate에 값이 true면 중복됐다는 뜻이니까 여기로옴
          ...
          // 만약 회원가입 단계가 verification 단계일 때 실행
          // 인증코드 검증
      ...
      // 로그인 모드: 네트워크 ,백엔드 서버, 비밀번호 불일치, 사용자 없을시 여기로옴
      ...
      if (mode === "login") {
        ...
        const msg =
          err instanceof Error && err.message.includes("TURNSTILE_REQUIRED")
  ...
  const handleEmailCheck = async () => {
    // 재할당 불가 비동기 함수 선언 // 회원가입 폼에서 "중복확인" 버튼 클릭 시 실행됨
    if (!email) {
      // email이 falsy값이면 true // falsy값은 "", 0, null, undefined, false, NaN 등
      setError("이메일을 입력해주세요.");
      return;
    }
    if (!EMAIL_REGEX.test(email)) {
      // 형식이 올바르지 않으면 여기서 차단(진행 막음)
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        "인증코드가 이메일로 발송되었습니다. 이메일을 확인해주세요.",
      );
      setRegisterStep("verification"); // 회원가입단계를 이메일 인증 단계로 변경
    } catch (err) {
  ...
  const switchToRegister = () => {
    // 재할당 불가 함수 // 회원가입 폼에 진입할때 모드와 단계를 초기화하는 함수
  ...
  const switchToLogin = () => {
    // 재할당 불가 함수 // 로그인 폼에 진입할때 모드를 로그인 모드로 변경하는 함수
  ...
  const resetForm = () => {
    // 재할당 불가 함수 // 처음부터 다시 시작 버튼 클릭 시 실행
  ...
  return (
    ...
    <div className={styles.emailAuthOverlay}>
      {" "}
      {/* 젤 바깥 div에 emailAuthOverlay css 클래스스적용*/}
      <div className={`${styles.emailAuthContainer} ${styles.emailAuthForm}`}>
        ...
        <div className={styles.formHeader}>
          {" "}
          {/* formHeader css 클래스 적용*/}
          <h2 className={styles.formTitle}>
            {" "}
            {/* formTitle css 클래스 적용*/}
            {mode === "login" ? "로그인" : "회원가입"}{" "}
            {/* mode가 login이면 로그인 아니면 회원가입으로 표시시*/}
          </h2>
          <button // 버튼
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          {" "}
          {/* 폼 제출 시 handleSubmit 함수 실행 // form css 클래스 적용*/}
          <div className={styles.inputGroup}>
            {" "}
            {/* inputGroup css 클래스 적용*/}
          ...
          <button
            ...
            disabled={
              ...
              (mode === "register" &&
                ...
                !verificationCode) ||
              (mode === "login" && showTurnstile && !turnstileToken)
            }
          ...
          >
            ...
            {
              isLoading
                ? "처리중..." // isLoading이 true면 처리중... 표시
                : mode === "login"
                  ? "로그인" // mode가 login이면 로그인 표시
                  : registerStep === "email"
                    ? "다음" // registerStep이 email이면 다음 표시
                    : registerStep === "verification"
                      ? "인증하기"
                      : "회원가입"
        ...
        <div className={styles.formFooter}>
          ...
          {mode === "login" ? ( // mode가 login이면 렌더링링
            ...
            >
              이미 계정이 있으신가요? 로그인{" "}
              {/* 위에 동작을 이미 계정이 있으신가요? 로그인 버튼에 연결함*/}
            </button>
          )}
        </div>
