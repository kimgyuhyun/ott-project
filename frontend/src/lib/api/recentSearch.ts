...
export async function fetchRecentSearches(): Promise<string[]> {
  ...
  try {
    ...
    console.info(
      `[Search][Recent] GET end ${duration}ms len=${result.length} cache=false`,
    );
    return result;
  } catch (error) {
    const duration = Date.now() - startTime;
...
export async function addRecentSearch(term: string): Promise<string[]> {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.info(
      `[Search][Recent] POST end ${duration}ms len=${result.length}`,
    );
    return result;
  } catch (error) {
    const duration = Date.now() - startTime;
  ...
  try {
    const result = await deduplicateRequest("removeRecentSearch", async () => {
      return await apiCall<string[]>(
        `/api/search/recent?term=${encodeURIComponent(term)}`,
        {
          method: "DELETE",
        },
      );
    ...
    const duration = Date.now() - startTime;
    console.info(
      `[Search][Recent] DELETE end ${duration}ms len=${result.length}`,
    );
    return result;
  } catch (error) {
    const duration = Date.now() - startTime;
    console.error(
      `[Search][Recent] DELETE failed ${duration}ms error=${error}`,
    );
    throw error; // 쓰기 실패는 에러 throw
  }
}
...
export async function clearRecentSearches(): Promise<void> {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.error(
      `[Search][Recent] DELETE ALL failed ${duration}ms error=${error}`,
    );
    throw error; // 쓰기 실패는 에러 throw
  }
}
