...
export async function getSearchSuggestions(
  ...
  const raw = await apiCall(
    `/api/search/suggest?query=${encodeURIComponent(query.trim())}&limit=${limit}`,
  );
...
export async function searchContent(
  ...
  page: number = 0,
  size: number = 20,
): Promise<SearchResultItem[] | PagedResponse<SearchResultItem>> {
  ...
  return apiCall<SearchResultItem[] | PagedResponse<SearchResultItem>>(
    `/api/search?${params.toString()}`,
  );
}

// 애니메이션 전용 검색 (기존 anime.ts와 중복 방지)
export async function searchAnimeOnly(
  query: string,
  page: number = 0,
  size: number = 20,
): Promise<SearchResultItem[]> {
  const raw = await apiCall(
    `/api/anime/search?query=${encodeURIComponent(query)}&page=${page}&size=${size}`,
  );
  return normalizeArray<SearchResultItem>(raw);
}
