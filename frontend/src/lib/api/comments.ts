...
async function apiCall<T>(
  ...
  if (!contentType || !contentType.includes("application/json")) {
    console.log("🌐 [FRONTEND] API 성공 (빈 응답):", {
      status: response.status,
    });
    return {} as T; // 빈 객체 반환
  }
...
export async function getReviewComments(
  ...
  size: number = 10,
  sort: string = "latest",
): Promise<PagedResponse<ReviewComment>> {
  return apiCall<PagedResponse<ReviewComment>>(
    `/api/reviews/${reviewId}/comments?page=${page}&size=${size}&sort=${sort}`,
  );
}
