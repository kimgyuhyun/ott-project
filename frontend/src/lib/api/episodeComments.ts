...
import {
  EpisodeComment,
  PagedEpisodeCommentsResponse,
} from "@/types/episodeComments";
...
export async function getEpisodeComments(
  ...
  size: number = 10,
  sort: string = "latest",
): Promise<PagedEpisodeCommentsResponse> {
  return apiCall(
    `/api/episodes/${episodeId}/comments?page=${page}&size=${size}&sort=${sort}`,
  );
}
...
// 댓글 수정
export async function updateEpisodeComment(
  episodeId: number,
  commentId: number,
  commentData: {
    content: string;
...
// 댓글 좋아요 토글
export async function toggleEpisodeCommentLike(
  episodeId: number,
  commentId: number,
) {
  console.log("🌐 [FRONTEND] toggleEpisodeCommentLike 호출:", {
    episodeId,
    commentId,
  });
...
// 댓글 신고
export async function reportEpisodeComment(
  episodeId: number,
  commentId: number,
) {
...
// 대댓글 목록 조회
export async function getEpisodeCommentReplies(
  episodeId: number,
  commentId: number,
...
// 대댓글 작성
export async function createEpisodeReply(
  episodeId: number,
  commentId: number,
  content: string,
): Promise<number> {
