...
export function createOrUpdateRating(animeId: number, score: number) {
  ...
  return apiCall<number>(
    `/api/anime/${animeId}/ratings?score=${encodeURIComponent(fixed)}`,
    {
      method: "POST",
    },
  );
...
export function getRatingStats(animeId: number) {
  console.log(
    "📡 getRatingStats call ->",
    `/api/anime/${animeId}/ratings/stats`,
  );
  return apiCall<{ distribution: Record<string, number>; average: number }>(
    `/api/anime/${animeId}/ratings/stats`,
  );
}
