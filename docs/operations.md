...
| `BillingItemsFailing` | 마지막 배치의 청구 실패 구독 수 > 0 (15분) | 정상값이 정의상 0. 카드 거절은 던닝이 처리하는 정상 결과라 여기 잡히지 않는다 |
| `PaymentReconcileInconclusive` | 결제 대사 판정 불가 건수 30분 증가분 > 0 (`batch` 별) | 정상값이 정의상 0. 환불 선점은 정리 못 한 건, PENDING 은 결제사 조회 실패(404 제외)·상태 판독 불가·금액 불일치·예외를 센다. 결제창 이탈과 ready 는 정상 미결이라 세지 않는다. 24시간을 넘긴 PENDING 은 결제사에 기록이 없거나 ready 면 CANCELED 로 닫고, 결론을 못 내면 매 주기 다시 센다 |
...
배치는 구독 한 건이 예외로 터져도 나머지를 계속 청구한다(한 사람의 이상한 데이터가 전체 매출을 막지 않게 하려고). 그 격리가 완주 신호를 무디게 만들기 때문에 — 전원이 실패해도 완주는 완주다 — `billing_batch_failed_items` 를 짝으로 둔다. 위가 "돌았는가", 아래가 "제대로 됐는가" 다. 둘 중 하나만 있으면 감시에 구멍이 남는다.

두 식 모두 `max_over_time(...[30m])` 으로 감싼다. 이 지표들은 인스턴스마다 값이 다르고(ShedLock 때문에 배치를 실제로 돈 쪽만 갱신된다) `max` 로 접는데, 롤링 배포가 인스턴스를 순차 교체하는 동안 최신값을 들고 있던 시계열이 잠깐 사라지면 `max` 가 과거로 후퇴한다. 2026-08-11 04:09 에 `BillingBatchStalled` 가 이 경로로 한 평가 주기만 firing 해 디스코드까지 나갔다(그 시각 배치는 정상이었다). 창은 사라진 시계열의 마지막 값을 살려두는 용도이고, 값이 늙는 것 자체는 그대로 보이므로 진짜 정지는 늦게 잡히지 않는다.
...
| 배포 실패 | Prometheus 지표가 아니다. CD(GitHub Actions) 실패는 워크플로 알림 경로에서 잡아야 한다 | 채널은 정해졌다(디스코드 웹훅, 07fc30e). 남은 것은 워크플로에서 그 웹훅을 부르는 일 |

**알려진 한계(후속 과제)**

- **발송은 5분 주기 폴링이라 지연과 과잉 전달이 함께 있다**: Alertmanager 대신 `security/ott-alert-relay.ps1` 이 `/api/v1/alerts` 를 5분마다 읽어 디스코드로 넘긴다(07fc30e). 그래서 firing 부터 알림까지 최대 5분이 비고, 반대로 한 평가 주기만 뜬 경보도 그 순간에 걸리면 그대로 나간다 — 2026-08-11 04:09 의 `BillingBatchStalled` 오탐이 이 경로였다. 규칙에 `for` 를 붙여 릴레이가 줍기 전에 걸러내는 것이 현재 방어이고, 억제·그룹핑·해제 알림은 여전히 없다
- **백엔드가 전부 내려가면 `BillingBatchStalled` 는 울리지 않는다**: 시계열 자체가 사라져 식이 아무것도 반환하지 않는다. 배치가 "죽은 것"은 잡지만 앱이 "없는 것"은 못 잡는다. `up{job="ott-backend"}` 기반 경보가 따로 필요하다
